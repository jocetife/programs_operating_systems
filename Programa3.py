...
class Process:
    def __init__(self, operator, op1, op2, TME, ide, TLlegada=0):
...
def BCP():
    ...
    bcp_text = tk.Text(frame, bg="grey", fg="white", wrap=tk.WORD, height=15, width=100, font=("Calibri", 12))
    ...
    bcp = "  ID       Operacion       Resultado       TLlegada       TFin       TServicio       TRetorno       TEspera       TRespuesta       TME "
    ...
    for p in fin_process:
        ...
        bcp += ("\n\n  " + str(p.ide)+ "\t " + str(p.op1) + str(p.operator) + str(p.op2) +"\t        " + str(resultado) + 
                "\t\t " + str(p.TLlegada) + "\t   " + str(p.TFin) + "\t    " + str(p.TT) + "\t          " + str(p.TRetorno) + 
                "\t\t" + str(p.TEspera)  + "\t        " + str(p.TRespuesta)  + "\t           " + str(p.TME) +  "\n")
